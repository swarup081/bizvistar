...
export default function UpdatePlanNavButton({ isMobile = false }) {
  ...
  if (isMobile) {
    return (
        <Link
            href={checkoutHref}
            className="flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-r from-[#0A1128] to-[#002B5E] text-white shadow-md hover:shadow-lg transition-all border border-blue-900/50"
            title={buttonText}
        >
