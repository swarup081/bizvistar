...
export default function ProductsPage() {
  ...
  return (
    <div className="h-full flex flex-col font-sans pb-20 md:pb-0">
      ...
      {/* Mobile Tab Switcher */}
      <div className="md:hidden relative flex p-1 bg-gray-100 rounded-full mb-4 mx-2 overflow-hidden">
          <div
            className={`absolute top-1 bottom-1 w-1/2 bg-white rounded-full shadow-sm transition-transform duration-300 ease-in-out ${
              activeTab === 'products' ? 'translate-x-0' : 'translate-x-[95%]'
            }`}
          />

          <button
            onClick={() => setActiveTab('products')}
            className="relative z-10 flex-1 py-1.5 text-xs font-bold text-center text-gray-900 transition-colors duration-200"
          >
            Products
          </button>
