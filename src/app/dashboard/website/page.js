    ...
    async function fetchUserWebsite() {
      try {
        ...
        if (slugParam) {
           ...
           // Default: Most recent (LIVE)
           query = query.eq('is_published', true).order('updated_at', { ascending: false }).limit(1).maybeSingle();
        }
        ...
        if (dbError) {
             ...
             setWebsite({
                 ...
                 // Prefer draft data for the editor view inside dashboard,
                 // but typically dashboard shows the live site status?
                 // The component is 'EditorLayout', so it's an editor.
                 // If we want to edit the LIVE site, we pass draft_data (which mirrors live + changes).
                 data: site.draft_data || site.website_data
             });
        } else {
            setError("You haven't published a website yet.");
        }
      } catch (err) {
